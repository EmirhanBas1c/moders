...
client.on('message', async message => {
  ...
  message.channel.send(`Bot Bilgi Kanallarının kurulumu başlatılsın mı? başlatılacak ise **kabul** yazınız. oGün Developments iyi eğlenceler diler`)
 ...
 channel.setParent(message.guild.channels.find(channel => channel.name === "✮ ▬  ▬ Duyuru Kanalları▬  ▬ ✮")));
  ...
  .then(c => {
    ...
    c.overwritePermissions(role, {
       ...
       message.guild.owner.send("Gerekli Herşey Kuruldu Rahatına Bak! **İbrahimYT  Bot'un kıymetini bil :D**")
...
.setTitle(`oGün Developments | Bilgilendirme`)
.setTimestamp()
.setColor("GREEN")
.setDescription(`Beni Sunucuna Eklediğin İçin Teşekkür Ederim \n Sana En İyi Şekilde Hizmet Edeceğim.\n Eğer Bir Sorunla Karşılaşırsan Destek Sunucuma Gel  https://discord.gg/Dcpadf4z7a \n Komutlarımız için **i!yardım** komutunu kullanınız.`)
              ...
              if (reklam.some(word => msg.content.toLowerCase().includes(word))) {
                try {
                  if (!msg.member.hasPermission("MANAGE_GUILD")) {
                    ...
                    let embed = new Discord.RichEmbed()
                    .setColor(0xffa300)
                    .setFooter('oGün Developments  -|-  Reklam engellendi.', client.user.avatarURL)
...
.setDescription(`<@!${message.author.id}> adlı kullanıcı tarafından <#${message.channel.id}> kanalına gönderilen mesaj silindi!\n\nSilinen Mesaj: **${message.content}**`)
  .setFooter("oGün Developments|  Mod-Log")
...
const ms = require('parse-ms')
  ...
  if (message.content.length > 1){ 
    db.set(`goldsure_${message.author.id}`, Date.now());
         ...
         .setColor('BLACK')
         .setTitle('Gold Üye!')
         .setDescription('<a:Heart_Rainbow:853075920844095508> **Bir gold üye belirdi!**')
